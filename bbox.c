...
/**
 * @brief Calculate bounding box for a netblock
 * 
 * Find the "bounding box" for the IPv4 netblock starting at 'first' and having
 * 'slash' netmask bits.
 *
 * For square areas this is pretty easy.  We know how to find the point
 * diagonally opposite the first value (add 1010..1010). Its a little harder for
...
/**
 * @brief Calculate the bounding box of a CIDR prefix string
 */
bbox bbox_from_cidr(const char *cidr) {
  ...
  if (debug) {
    ...
    fprintf(stderr, "cidr=%s"
                    ...
                    ", bbox=%d,%d to %d,%d"
                    "\n",
            cidr, fstr, lstr, last - first, bbox.xmin, bbox.ymin, bbox.xmax,
            bbox.ymax);
  }
  return bbox;
}
