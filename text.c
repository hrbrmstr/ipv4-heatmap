 ...
 */
static int *text_width_height(const char *text, double sz, int *w, int *h) {
  static int brect[8];
  char *errmsg = gdImageStringFT(NULL, &brect[0], 0, (char *)font_file_or_name,
 ...
 *
 * If the optional 'text2' is given, then it is drawn below the first text,
 * always
 * in a 12-point size.  This is mainly to print the CIDR prefix values
 * underneath their owner names in each /8 box.
 */
void text_in_bbox(const char *text, bbox box, int color, double maxsize) {
  double sz;
  int tw, th;
  int oneline_h;
  int *brectPtr;
  ...
  for (sz = maxsize; sz > 6.0; sz *= 0.9) {
    ...
    if (th > ((box.ymax - box.ymin) * 95 / 100))
      continue;
    gdImageStringFT(image, brectPtr, color, (char *)font_file_or_name, sz, 0.0,
