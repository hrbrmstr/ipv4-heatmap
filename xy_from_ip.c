...
void (*xy_from_s)(unsigned s, int n, unsigned *xp,
                  unsigned *yp) = hil_xy_from_s;
...
/**
 * @brief Calculate x+y coordinates from a numeric IPv4
 *
...
void set_morton_mode() { xy_from_s = mor_xy_from_s; }

int set_order() {
  hilbert_curve_order =
      (addr_space_bits_per_image - addr_space_bits_per_pixel) / 2;
  if (debug) {
    struct in_addr a;
    char buf[20];
...
void set_crop(const char *cidr) {
  cidr_parse(cidr, &addr_space_first_addr, &addr_space_last_addr,
             &addr_space_bits_per_image);
